...
answersPerGroup.forEach(answers => {
	//the Set will get only the distinct characters in  the group
	let uniqueAnswers = String.prototype.concat(...new Set(answers));
	totalCount += uniqueAnswers.length; //increase the totalCount of answers with the amount of distinct answers
});
console.log(totalCount);

//method 2 optimized
answersPerGroup.forEach(answers => totalCount += String.prototype.concat(...new Set(answers)).length);
console.log(totalCount);
...
answersPerGroup.forEach(group => {
	...
	let mergedAnswers = group.join(''); //join all the people's answers
	let charCount = {}; //an object to keep track of the answer count

	mergedAnswers.split('').forEach(x => {
		//if the answer has already been given increment it's count
		//else set the count to 1
		if(charCount.hasOwnProperty(x)) {
			charCount[x]++;
		} else {
			charCount[x] = 1;
		}
	...
	//foreach given answer, check if the answer has only been given once
	//if so, increase the total count
	Object.keys(charCount).forEach(key => {
		if(charCount[key] === groupCount) {
			totalCount++;
		}
	});
})
