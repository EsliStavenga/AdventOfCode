const _bags = `dark maroon bags contain 2 striped silver bags, 4 mirrored maroon bags, 5 shiny gold bags, 1 dotted gold bag.
...
bags = _bags.split('\n')
	.map(x => {
		return {
			parent: x.substr(0, x.indexOf('contain ') - 6),
			children: x.substr(x.indexOf('contain ') + 'contain '.length).split(',').map(x => x.substr(2, x.indexOf('bag') - 3).trim())
		}
	});
...
do {
	change = false;
	bags.forEach(bag => {
		if(bag.children[0] === 'other') { //contain no other bags
			return;
		}

		//check if one of the bag's children is also a parent of a bag that contains shiny gold
		let child = false;
		bag.children.forEach(childBag => {
			child = child || parents.indexOf(childBag) >= 0;
		});

		//if either the parent or one of it's children can contain a shiny gold, save it as a root parent
		if((bag.children.indexOf("shiny gold") >= 0 || child) && parents.indexOf(bag.parent) === -1) {
...
//part 2
//turns the input into a parsed array formatted [{ parent: "shiny gold", children: [{child: "red", count: 2}, {child: "white", count: 12}, ...] }, ...]
bags = _bags.split('\n').map(x => { return {parent: x.substr(0, x.indexOf('contain ') - 6), children: x.substr(x.indexOf('contain ') + 'contain '.length).split(',').map(x => { return {child: x.substr(2, x.indexOf('bag') - 3).trim(), count: parseInt(x.trim().substr(0, 1))} })}});

const shiny = getParentObject('shiny gold');
console.log('abc', abc(shiny) - 1);

function abc(bag) {
	let count = 1;

	//loop over all the children and calculate teh product
	if(bag.children && bag.children[0].child !== 'other') {
		bag.children.forEach(x => {
			count += abc(getParentObject(x.child)) * x.count;
		});
	}
...
//get the parent object based off the code
function getParentObject(parent) {
	return bags.filter(x => x.parent === parent)[0];
}
