...
14-16 p: tpkppppppppppppppppp`.split('\n');

//part 1
let countValid = 0;

passwords.forEach(x => {
	let lowerBound = x.substr(0, x.indexOf('-'));
	let upperBound = x.substr(x.indexOf('-') + 1, x.indexOf(' ') - x.indexOf('-'));
	let char = x.substr(x.indexOf(' ') + 1, 1);
	let matchCount = (x.substr(x.indexOf(' ') + 3).match(new RegExp(char, 'g')) || []).length;
...
passwords.forEach(x => {
	...
	let upperBound = x.substr(x.indexOf('-') + 1, x.indexOf(' ') - x.indexOf('-'));
	let char = x.substr(x.indexOf(' ') + 1, 1);
	let string = (x.substr(x.indexOf(' ') + 3));

	let firstChar = string.substr(lowerBound, char.length);
	let secondChar = string.substr(upperBound, char.length)
