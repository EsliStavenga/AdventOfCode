const passwords = `eyr:2029 iyr:2013
...
passwords.forEach((password) => {
	...
	let keyValuePairs = password.split(' ');
	let keyValues = {};
	...
	requiredFields.forEach(field => {
		//if the passport has all required fields it's deemed valid
		if(Object.keys(keyValues).indexOf(field) < 0) {
			isValid = false;
		}
...
//part 2 SECOND TRY, first try was 0, you never know...

//all required fields with the allowed values
//special checks for pid and hcl
const requiredFields = {
	byr: [...Array(2003).keys()].filter(x => x >= 1920).map(x => x.toString()),
	iyr: [...Array(2021).keys()].filter(x => x >= 2010).map(x => x.toString()),
	eyr: [...Array(2031).keys()].filter(x => x >= 2020).map(x => x.toString()),
	hgt: ([...Array(194).keys()].filter(x => x >= 150).map(x => `${x}cm`)).concat([...Array(77).keys()].filter(x => x >= 59).map(x => `${x}in`)).map(x => x.toString()),
	hcl: [...Array(10).keys()].concat('abcdef'.split('')).map(x => x.toString()),
	ecl: ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].map(x => x.toString())
// 	pid:
};

const requiredFieldKeys = Object.keys(requiredFields);
requiredFieldKeys.push('pid');
...
passwords.forEach((password) => {
	...
	let keyValuePairs = password.split(' ');
	let keyValues = {};
	//split the key value pairs into an object
	// { field: value }
	keyValuePairs.forEach(x => {
		let a = x.split(':');
		keyValues[a[0]] = a[1];
	...
	requiredFieldKeys.forEach(key => {
		//not worth looping if the password is already deemed invalid
		if(!isValid) {
			return;
		}

		//if the password doesn't have all the required fields
		if(!Object.keys(keyValues).includes(key)) {
			isValid = false;
			//special check for PID
		} else if(key === 'pid') { //can't be merged cause pid is not in requiredFields
			if((keyValues[key].match(/^[0-9]*$/g) || []).length !== 1 || keyValues[key].length !== 9) {
				isValid = false;
			}
			//special check for HCL
		} else if(key === 'hcl') {
			const a = keyValues[key].split('');
			if(a.shift() !== '#') { //haircolor must be a hex value
				isValid = false;
				return;
			}

			//only 0-9 and a-f are valid chracters
			a.forEach(hclLetter => {
				if(!requiredFields[key].includes(hclLetter)) {
					isValid = false;
				}
			});

			//just check if the value is in the allowed values array thingy we generate at the start
		} else if(!requiredFields[key].includes(keyValues[key])) {
			isValid = false;
		}
	});
